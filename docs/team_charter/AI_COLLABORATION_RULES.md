# AI Design Studio Kit AI Collaboration Rules
...
This document defines how humans and AI agents collaborate on AI Design Studio Kit.
...
- Final QA verdicts.
- GitHub publishing.
- Setting a task's status to `Done` after integrating the QA Agent's recommended verdict and verifying evidence. This is the only role that may make that transition. **Exception for this kit:** every track's Done decision (Visual Design, Brand Identity, Marketing Templates, UX) is always Hard-Gate tier — see `STANDING_APPROVAL_LANES.md` — so the Main Conductor may set a design task's status to `Done` only after David's explicit sign-off, never on a Standard-tier QA-Pass alone. This matches `ARCHITECTURE.md`'s "David is the Done Decider for every track, every time," enforced here through the kit's own Hard-Gate mechanism rather than as a special-case override of it.
